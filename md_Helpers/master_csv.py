...
def _seitz_nan_terms():
    return {
        "N_cav": np.nan,
        "u_c": np.nan,
        "u_c_uncertainty": np.nan,
        "rho_liquid": np.nan,
        "rho_liquid_uncertainty": np.nan,
        "rho_cav": np.nan,
        "P_EOS": np.nan,
        "P_EOS_uncertainty": np.nan,
        "P_EOS_mean_uncertainty": np.nan,
        "P_EOS_density_uncertainty": np.nan,
        "P_EOS_density_slope": np.nan,
        "u_EOS": np.nan,
        "u_EOS_uncertainty": np.nan,
        "u_EOS_mean_uncertainty": np.nan,
        "u_EOS_density_uncertainty": np.nan,
        "u_EOS_density_slope": np.nan,
        "Q": np.nan,
        "Q_uncertainty": np.nan,
        "Q_uncertainty_u_c_component": np.nan,
        "Q_uncertainty_u_EOS_component": np.nan,
        "Q_uncertainty_P_EOS_component": np.nan,
        "Q_uncertainty_rho_liquid_component": np.nan,
    }
...
def summarize_seitz_cavitation_log(
    ...
    try:
        ...
        row.update({
            ...
            "u_c_uncertainty": terms.get("uc_uncertainty", np.nan),
            "rho_liquid": terms.get("rho_0", np.nan),
            "rho_liquid_uncertainty": terms.get(
                "rho_0_uncertainty",
                np.nan,
            ),
            "rho_cav": terms.get("rho_c", np.nan),
            "P_EOS": terms.get("P0", terms.get("p0", np.nan)),
            "P_EOS_uncertainty": terms.get("P0_uncertainty", np.nan),
            "P_EOS_mean_uncertainty": terms.get(
                "P0_mean_uncertainty",
                np.nan,
            ),
            "P_EOS_density_uncertainty": terms.get(
                "P0_density_uncertainty",
                np.nan,
            ),
            "P_EOS_density_slope": terms.get("P0_density_slope", np.nan),
            "u_EOS": terms.get("u0", np.nan),
            "u_EOS_uncertainty": terms.get("u0_uncertainty", np.nan),
            "u_EOS_mean_uncertainty": terms.get(
                "u0_mean_uncertainty",
                np.nan,
            ),
            "u_EOS_density_uncertainty": terms.get(
                "u0_density_uncertainty",
                np.nan,
            ),
            "u_EOS_density_slope": terms.get("u0_density_slope", np.nan),
            "Q": terms.get("Q", terms.get("q_seitz", np.nan)),
            "Q_uncertainty": terms.get("Q_uncertainty", np.nan),
            "Q_uncertainty_u_c_component": terms.get(
                "Q_uncertainty_uc_component",
                np.nan,
            ),
            "Q_uncertainty_u_EOS_component": terms.get(
                "Q_uncertainty_u0_component",
                np.nan,
            ),
            "Q_uncertainty_P_EOS_component": terms.get(
                "Q_uncertainty_P0_component",
                np.nan,
            ),
            "Q_uncertainty_rho_liquid_component": terms.get(
                "Q_uncertainty_rho_0_component",
                np.nan,
            ),
        })
        return row
