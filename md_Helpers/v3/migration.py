...
from . import paths as v3_paths
from . import metadata as v3_metadata
from .io import copy_or_link_file
...
def migrate_thermalized_states(
    ...
    for old_log_path in log_paths:
        ...
        new_paths = v3_paths.thermalized_run_paths(
        ...
        pairs = [
            (old_state_path, new_paths["state_path"], "state_gsd"),
            (old_log_path, new_paths["log_path"], "log_hdf5"),
        ]

        for source_path, destination_path, file_role in pairs:
            row = {
                "object_kind": "thermalized",
                "file_role": file_role,
...
def migrate_cavitation_states(
    ...
    for old_state_path in state_paths:
        ...
        new_paths = v3_paths.cavitation_state_paths(
            n_fcc_cells=info["n_fcc_cells"],
            source_rho=info["rho"],
            kT=info["kT"],
        ...
        v3_metadata.write_creation_metadata(
...
def migrate_v2_to_v3(
    ...
    migrate_cavitation_states=True,
    limit=None,
):
    """
    Copy or link existing V2 saved files into the V3 layout.
